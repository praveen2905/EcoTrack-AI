...
export default function ChallengesPage() {
  ...
  const completeChallenge = useMutation({
    mutationFn: async (id) => {
      const res = await fetch(`/api/challenges/${id}/complete`, { method: "PATCH" });
              ...
              <Card key={challenge.id} className={`flex flex-col transition-all ${challenge.completed ? "bg-muted/30 border-muted" : "hover:shadow-md border-primary/20"}`}>
                <CardHeader>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="flex items-center text-xs font-medium text-amber-600 dark:text-amber-400 bg-amber-500/10 px-2 py-1 rounded-full">
                ...
                <CardContent className="flex-1 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-4 mt-4 text-xs font-medium">
                    <span className="flex items-center text-primary"><Leaf className="w-3 h-3 mr-1" />-{challenge.carbonSaved} kg CO₂ saved</span>
                ...
                <CardFooter>
                  <Button className="w-full" variant={challenge.completed ? "secondary" : "default"} disabled={challenge.completed || completeChallenge.isPending} onClick={() => completeChallenge.mutate(challenge.id)}>
